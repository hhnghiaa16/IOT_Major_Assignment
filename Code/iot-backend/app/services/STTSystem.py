...
<<<<<<< HEAD
    ...
    def _build_proxy_client(self, proxy_url: Optional[str], username: Optional[str], password: Optional[str]) -> httpx.Client:
        """
        Tạo httpx.Client kèm proxy nếu được cấu hình.
        Cho phép chèn username/password vào URL mà không cần xác minh chứng chỉ proxy.
        """
        if not proxy_url:
        ...
        if not parsed.scheme:
            raise ValueError("Proxy URL phải bao gồm scheme, ví dụ socks5:// or http://")
